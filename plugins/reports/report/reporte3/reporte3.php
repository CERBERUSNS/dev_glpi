...
Html::header(__('reporte3_report_title', 'reports'), $_SERVER['PHP_SELF'], "utils", "report");
...
echo "<tr class='tab_bg_2'><th>". __('Fecha de Modificacion', 'reports') . "</th>" .
      "<th>". __('Categoria','reports') . "</th>".
      "<th>". __('Totales','reports') . "</th>"
      //"<th>". __("Computer's name") . "</th>".
      //"<th>". sprintf(__('%1$s (%2$s)'), _n('Software', 'Software', 1), __('version'))."</th></tr>\n
      ;

$sql = "call Proc_Lista3(date(now()),last_day(now()))";
...
*/     
      ...
      echo "<tr class='" . $class . "_top'>".
            "<td class='left'>".($data["FechaDeModificacion"]) . "</td>" .
            "<td class='left'>".($data["Categoria"]) . "</td>" .
            "<td class='center'>".($data["TotalesAbiertosMesActual"]) . "</td>" ;

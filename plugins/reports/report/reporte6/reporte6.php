...
Html::header(__('Reporte6_report_title', 'reports'), $_SERVER['PHP_SELF'], "utils", "report");
...
echo "<tr class='tab_bg_2'><th>". __('Abiertos del Mes', 'reports') . "</th>" .
      "<th>". __('Solucionados el Mismo Mes','reports') . "</th>".
      "<th>". __('Solucionados del Mes Anterior','reports') . "</th>".
      "<th>". __('Atendidos en el Mes','reports') . "</th>".
      "<th>". __('Sin Resolver en el Mes','reports') . "</th>".
      "<th>". __('Sin Resolver Acumulado','reports') . "</th>"
      //"<th>". __("Computer's name") . "</th>".
      //"<th>". sprintf(__('%1$s (%2$s)'), _n('Software', 'Software', 1), __('version'))."</th></tr>\n
      ;

$sql = "call proc_general_1(date(now()),last_day(now()))";
...
*/     
      ...
      echo "<tr class='" . $class . "_top'>".
            "<td class='center'>".($data["AbiertosMes"]) . "</td>" .
            "<td class='center'>".($data["SoluMismoMes"]) . "</td>" .
            "<td class='center'>".($data["SoluDelMesAnterior"]) . "</td>" .
            "<td class='center'>".($data["AtendidosEnElMes"]) . "</td>" .
            "<td class='center'>".($data["SinResolverMes"]) . "</td>" .
            "<td class='center'>". $data["SinResolverAcumulado"] . "&nbsp;</td>";

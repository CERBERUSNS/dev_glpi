...
class PluginAdditionalalertsInfocomAlert extends CommonDBTM {
   ...
   static function getTypeName() {
      ...
      return $LANG['plugin_additionalalerts']['alert'][11];
   }
   ...
   static function cronInfo($name) {
      ...
      switch ($name) {
         case 'AdditionalalertsNotInfocom':
            return array (
               'description' => $LANG['plugin_additionalalerts']['alert'][11]);   // Optional
            break;
      }
      return array();
   }
      ...
      foreach (self::getEntitiesToNotify('use_infocom_alert') as $entity => $repeat) {
         ...
         foreach ($DB->request($query_notinfocom) as $data) {
            ...
            if (!isset($notinfocoms_infos[$type][$entity])) {
               $notinfocom_messages[$type][$entity] = $LANG['plugin_additionalalerts']['alert'][11]."<br />";
            }
   ...
   static function configCron($target,$ID) {
      ...
      echo "<td>".$LANG['plugin_additionalalerts']['setup'][29]." : ";
      Dropdown::show('ComputerType', array('name' => "types_id"));
      echo "&nbsp;<input type='submit' name='add_type' value=\"".$LANG['buttons'][2]."\" class='submit' ></div></td>";
      echo "</tr>";
   ...
   static function showNotificationOptions(Entity $entity) {
      ...
      echo "<tr><th colspan='2'>".$LANG['setup'][242]."</th></tr>";

      echo "<tr class='tab_bg_1'><td>" . $LANG['plugin_additionalalerts']['alert'][11] . "</td><td>";
